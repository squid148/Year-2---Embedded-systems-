...
char codeSequence[NUMBER_OF_KEYS]   = { '1', '8', '0', '5' }; //passcode array to store correct passcode
...
void alarmdeactivate(){
    ...
    if (KeyReleased != '\0' && KeyReleased !='#' && KeyReleased != '*'){ //key isn't null and # (enter), can't be or otherwise it would always be true in idle
        ...
        } else {KeypadCodeIndex++;}//increase index by 1 and populate next member
...
void codeprint(){
    char str[8]; int StringLength;//might not work but unsure need to test
