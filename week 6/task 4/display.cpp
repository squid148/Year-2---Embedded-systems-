...
void LCDStartupPrompt() {
    bool ContinuePress=OFF; //I'm going to use this as a flag for a loop so the user has to press something to continue
    char key; //unsure if this will cause problems with other variable called key but this is being used to change the flag state and exit the loop when needed
    //it's a local variable so it shouldn't cause an issue
    displayCharPositionWrite(0, 0); //print out a message on lcd screen
    displayStringWrite("Enter code to ");
    displayCharPositionWrite(0, 1);
    displayStringWrite("deactivate alarm.");
    displayCharPositionWrite(0, 2);
    displayStringWrite("Press any Keypad");
    displayCharPositionWrite(0, 3);
    displayStringWrite("key to continue.");
    while (!ContinuePress) {
        key=KeypadUpdate();//updating trigger
        if (key != '\0'){ //if any key is pressed so key becomes null
            ContinuePress=ON;//break out of loop by changing flag to on
        }
        else {ContinuePress=OFF;}// else ensure it remains off and loops
    }
    displaywipe(); //wipe the display to set up for later
    //don't put a LCD report here, breaks update system
}
