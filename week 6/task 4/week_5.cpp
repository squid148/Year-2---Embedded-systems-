...
char codeSequence[NUMBER_OF_KEYS]   = { '2', '9', '4', '3','5' }; //passcode array to store correct passcode
char LCDReportentry[NUMBER_OF_KEYS] = {'4','5', '0', '0','0'};//added this code so the user can input 4 and 5 respectively and enter to get LCD report, W6_task 1 related,
...
char keyPressed[NUMBER_OF_KEYS] = { '0', '0', '0', '0', '0'}; //stores user input
    ...
    if (KeyReleased ==  'D'){ //enter route
         ...
         if (alarmState){
                if (areEqual()) { //if code is correct reset system
                    ...
                    displaywipe();
                    delay(10000); //freeze for 30 seconds
                } 
                }
                else {
...
void codeprint(){
    ...
    sprintf(str,"\rentered code: %c%c%c%c%c\n---------------------------\n", keyPressed[0],keyPressed[1],keyPressed[2],keyPressed[3],keyPressed[4]); //code is 1805
            StringLength=strlen(str);
            uartUsb.write(str,StringLength);
}
