...
// === [definitions have been put into headers to categorise them and clean up main]==

//=====[decleration and initialization of public global objects moved to respective header files]
//code is 29435, 5 digits have been implemented
...
int main() {  
    ...
    while (true) {
        ...
        alarmdeactivate(); 
        if (!time_set){
        timesetter();// function asks user to set time once at the beginning of the program before continuing
        ...
        if (alarmState){
            buzzer();// enables buzzer in alarm conditions and states the reason
            codeprint(); //prints entered code
        }
        else {
        sirenPin=OFF; /*sirepin is now acknowledged by main, have to use external declare on some variables
